...
function parseAdapter(value: unknown, label: string): CreSimulatorAdapterKey {
	if (
		value === "auto-reveal-relayer" ||
		value === "cre-workflow-simulate" ||
		value === "jury-orchestrator-run-once" ||
		value === "demo-adjudication-orchestrator"
	) {
		return value;
	}
...
function parseCreWorkflowSimulateAdapterConfig(
	...
	if (!isObject(value)) {
		throw new Error(
			`Invalid cre-workflow-simulate config for trigger=${triggerName}`,
		);
	}
	if (typeof value.workflowPath !== "string") {
		throw new Error(
			`cre-workflow-simulate trigger ${triggerName} must define workflowPath`,
		);
	}
	if (typeof value.target !== "string" || value.target.trim().length === 0) {
		throw new Error(
			`cre-workflow-simulate trigger ${triggerName} must define target`,
		);
	}
	if (
		...
		throw new Error(
			`cre-workflow-simulate trigger ${triggerName} must define a non-negative triggerIndex`,
		);
	}
	if (value.evmInput !== undefined && value.evmInput !== "event-coordinates") {
		throw new Error(
			`cre-workflow-simulate trigger ${triggerName} has unsupported evmInput`,
		);
	}
	ensureRepoScopedPath(repoRoot, value.workflowPath, "workflowPath");
	const idempotencyStorePath =
		typeof value.idempotencyStorePath === "string"
			? (() => {
					ensureRepoScopedPath(
						repoRoot,
						value.idempotencyStorePath,
						"idempotencyStorePath",
					);
					return value.idempotencyStorePath;
				})()
			: undefined;
	return {
		...
		...(value.evmInput === "event-coordinates"
			? { evmInput: value.evmInput }
			: {}),
		...(idempotencyStorePath ? { idempotencyStorePath } : {}),
	};
}
...
function parseAutoRevealAdapterConfig(
	...
	if (!isObject(value)) {
		throw new Error(
			`Invalid auto-reveal-relayer config for trigger=${triggerName}`,
		);
	}
	const configPath =
		typeof value.configPath === "string"
			? (() => {
					ensureRepoScopedPath(repoRoot, value.configPath, "configPath");
...
function parseJuryRunOnceAdapterConfig(
	repoRoot: string,
	triggerName: string,
	value: unknown,
): JuryOrchestratorRunOnceAdapterConfig {
	if (!isObject(value)) {
		throw new Error(
			`Invalid jury-orchestrator-run-once config for trigger=${triggerName}`,
		);
	}
	if (typeof value.configPath !== "string") {
		throw new Error(
			`jury-orchestrator-run-once trigger ${triggerName} must define configPath`,
		);
	}
	ensureRepoScopedPath(repoRoot, value.configPath, "configPath");
	return {
...
function parseDemoAdjudicationAdapterConfig(
	repoRoot: string,
	triggerName: string,
	value: unknown,
): DemoAdjudicationOrchestratorAdapterConfig {
	if (!isObject(value)) {
		throw new Error(
			`Invalid demo-adjudication-orchestrator config for trigger=${triggerName}`,
		);
	}
	if (typeof value.configPath !== "string") {
		throw new Error(
			`demo-adjudication-orchestrator trigger ${triggerName} must define configPath`,
		);
	}
	ensureRepoScopedPath(repoRoot, value.configPath, "configPath");
	const pocstorePath =
		typeof value.pocstorePath === "string"
			? (() => {
				ensureRepoScopedPath(repoRoot, value.pocstorePath, "pocstorePath");
				return value.pocstorePath;
			})()
			: undefined;
	return {
		configPath: value.configPath,
		...(pocstorePath ? { pocstorePath } : {}),
	};
}
...
function parseAdapterBinding(
	...
	if (adapter === "jury-orchestrator-run-once") {
		return {
			adapter,
			adapterConfig: parseJuryRunOnceAdapterConfig(
				repoRoot,
				triggerName,
				record.adapterConfig,
			),
	...
	if (adapter === "demo-adjudication-orchestrator") {
		return {
			adapter,
			adapterConfig: parseDemoAdjudicationAdapterConfig(
				repoRoot,
				triggerName,
				record.adapterConfig,
			),
	...
	return {
		...
		...(record.adapterConfig !== undefined
			? {
					adapterConfig: parseAutoRevealAdapterConfig(
						repoRoot,
						triggerName,
						record.adapterConfig,
					),
...
function parseBaseMapping(
	...
	return {
		...
		...(typeof record.evidenceDir === "string"
			? {
					evidenceDir: ensureRepoScopedPath(
						repoRoot,
						record.evidenceDir,
						"evidenceDir",
					),
...
function parseCronTriggers(
	...
	return Object.entries(value).map(([triggerName, record]) => {
		if (
			...
			throw new Error(
				`cron trigger ${triggerName} must define a positive intervalMs`,
			);
		}
		return {
			triggerName,
...
function parseEvmLogTriggers(
	...
	return Object.entries(value).map(([triggerName, record]) => {
		if (
			...
			throw new Error(
				`Invalid EVM log trigger config for trigger=${triggerName}`,
			);
		}
		return {
			triggerName,
...
export function buildDefaultCreSimulatorTriggerConfigPath(
	repoRoot: string,
): string {
...
export function loadCreSimulatorTriggerConfig(
	...
	if (
		...
		!statSync(resolvedConfigPath).isFile()
	) {
		throw new Error(
			`Trigger config file does not exist: ${resolvedConfigPath}`,
		);
	}
	...
	if (parsed.schemaVersion !== TRIGGER_CONFIG_SCHEMA_VERSION) {
		throw new Error(
			`Unsupported cre-simulator trigger config schema: ${String(parsed.schemaVersion)}`,
		);
	}
	...
	return {
		...
		stateFilePath: ensureRepoScopedPath(
			repoRoot,
			parsed.stateFilePath,
			"stateFilePath",
		),
