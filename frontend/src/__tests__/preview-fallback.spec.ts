...
describe('previewFallback helpers', () => {
  it('enables fallback by default in test mode', () => {
    expect(shouldUsePreviewFallback(undefined, 'test')).toBe(true)
  })
  ...
  it('formats a preview warning message for banner display', () => {
    expect(formatPreviewFallbackMessage('Failed to load projects from blockchain')).toContain(
      'Preview mode active'
    )
  })
})
