...
function buildProjectRow(params: {
	...
	const baseAddress =
		`0x${params.id.toString().padStart(40, "0")}` as `0x${string}`;
	return {
		owner: baseAddress,
...
describe("Landing featured projects", () => {
	...
	it("renders featured cards from on-chain project reads instead of hardcoded demo list", async () => {
		...
		mockPrimaryMulticall.mockResolvedValue([
			buildProjectRow({
				...
				active: true,
				mode: 0,
				bountyPool: 5_000_000_000_000_000_000n,
			}),
			buildProjectRow({
				id: 1n,
				active: false,
			...
			buildProjectRow({
				...
				active: true,
				mode: 1,
				bountyPool: 7_000_000_000_000_000_000n,
			}),
		]);
	...
	it("derives total bounty count and average reveal time from all active projects, not only featured cards", async () => {
		const nowMs = new Date("2026-03-08T00:00:00.000Z").getTime();
		const nowSeconds = BigInt(Math.floor(nowMs / 1000));
		vi.spyOn(Date, "now").mockReturnValue(nowMs);

		mockPrimaryReadContract.mockResolvedValue(5n);
		mockPrimaryMulticall.mockResolvedValue([
			{
				...buildProjectRow({
					id: 0n,
				...
				revealDeadline: nowSeconds + 2n * 86_400n,
			},
			{
				...buildProjectRow({
					id: 1n,
				...
				revealDeadline: nowSeconds + 4n * 86_400n,
			},
			{
				...buildProjectRow({
					id: 2n,
				...
				revealDeadline: nowSeconds + 10n * 86_400n,
			},
			{
				...buildProjectRow({
					id: 3n,
					active: true,
					mode: 1,
					bountyPool: 8_000_000_000_000_000_000n,
				}),
				revealDeadline: 0n,
			},
			{
				...
				revealDeadline: nowSeconds + 8n * 86_400n,
			},
		]);
		mockSecondaryReadContract.mockResolvedValue(5n);
		mockSecondaryMulticall.mockResolvedValue([
			{
				...buildProjectRow({
					id: 0n,
		...
		await waitFor(() => {
			...
			expect(screen.getByText("4d 16h")).toBeInTheDocument();
		});

		expect(screen.getByText("Active Projects")).toBeInTheDocument();
		expect(screen.getByText("Until reveal deadline")).toBeInTheDocument();
		expect(screen.queryByText("PROJECT_#4")).not.toBeInTheDocument();
	});
});
