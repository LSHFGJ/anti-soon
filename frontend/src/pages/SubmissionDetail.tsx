...
import { VerificationWorkflowPanel } from "../components/shared/VerificationWorkflowPanel";
import { buildVerificationWorkflowViewModel } from "../components/shared/verificationWorkflow";
...
export function SubmissionDetail() {
	...
	const isCurrentRequest = useCallback(
		(submissionId: bigint, requestId?: number) => {
	...
	const refreshSubmissionData = useCallback(
		async (submissionId: bigint, requestId?: number) => {
			...
			void artifactPromise.then(
				([fetchedProject, commitTxHash, sapphireTxHash]) => {
					if (!isCurrentRequest(submissionId, requestId)) {
						return;
					}
	...
	const handleChallenge = async () => {
		...
		try {
			...
			if (
				...
				now > submission.disputeDeadline
			) {
			...
			<div className="min-h-[calc(100vh-142px)] flex flex-col py-6">
				<div className="container flex-1 flex flex-col min-h-0">
					...
					<StatusBanner
						...
						message={`> ERROR: ${error || "Submission not found"}`}
					/>
	...
	const disputeWindowOpen = Boolean(
		submission.disputeDeadline > 0n && submission.disputeDeadline >= now,
	);
	const disputeWindowClosed = Boolean(
		submission.disputeDeadline > 0n && now > submission.disputeDeadline,
	);
	const isProjectOwner = Boolean(
		project && address && project.owner.toLowerCase() === address.toLowerCase(),
	);
	...
	const canFinalize = Boolean(
		(actualStatus === SUBMISSION_STATUS_VERIFIED ||
			actualStatus === SUBMISSION_STATUS_DISPUTED) &&
			disputeWindowClosed &&
			isConnected,
	);
	...
	const workflow = buildVerificationWorkflowViewModel({
			...
			<div className="container flex-1 flex flex-col min-h-0 overflow-y-auto">
				<PageHeader
					...
					suffix={
						<Badge variant={getStatusBadgeVariant()}>
							[{STATUS_LABELS[actualStatus as number]}]
						</Badge>
					}
				/>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
					<div className="flex flex-col gap-6">
						<NeonPanel
							className="border-[var(--color-bg-light)]"
							contentClassName="space-y-4 font-mono text-sm p-4"
						>
						...
						<NeonPanel
							className="border-[var(--color-bg-light)]"
							contentClassName="space-y-3 font-mono text-sm p-4"
						>
							<h2 className="text-sm font-mono text-[var(--color-secondary)] tracking-wider">
								TIMESTAMPS
							</h2>
						...
						>
							...
							{previewError ? (
								...
								<p className="text-[var(--color-text-dim)]">
									Load the Sapphire-stored PoC payload for this submission.
								</p>
							)}
						</NeonPanel>
					</div>
					...
					<div className="flex flex-col gap-6">
						<NeonPanel
							tone={submission.status >= 2 ? "primary" : "default"}
						...
						>
							...
							{submission.status < 2 ? (
								...
								<div className="space-y-4 font-mono text-sm">
									<MetaRow
										label="SEVERITY"
										value={<SeverityBadge severity={submission.severity} />}
										inline
									/>
									...
									<div className="flex justify-between pt-4 border-t border-[var(--color-bg-light)]">
										...
										<span
											className={`${submission.payoutAmount > 0n ? "text-[var(--color-primary)] font-bold text-base" : "text-[var(--color-text-dim)]"}`}
										>
											{submission.payoutAmount > 0n
												? `${formatEther(submission.payoutAmount)} ETH`
												: "N/A"}
										</span>
									</div>
									...
									<StatusBanner
										className="mt-4"
										variant={
											actualStatus === SUBMISSION_STATUS_INVALID ||
											hasActiveDispute
												? "error"
									...
									{submission.lifecycle && (
										<div className="pt-4 border-t border-[var(--color-bg-light)]">
											...
											<div className="space-y-3">
												<MetaRow
													label="STATUS"
													value={STATUS_LABELS[submission.lifecycle.status]}
													inline
												/>
												<MetaRow
													label="VERDICT_SOURCE"
													value={
														VERDICT_SOURCE_LABELS[
															submission.lifecycle.verdictSource
														]
													}
													inline
												/>
												<MetaRow
													...
													value={
														FINAL_VALIDITY_LABELS[
															submission.lifecycle.finalValidity
														]
													}
													inline
												/>
												{submission.lifecycle.juryDeadline > 0n && (
													<MetaRow
														label="JURY_DEADLINE"
														value={new Date(
															Number(submission.lifecycle.juryDeadline) * 1000,
														).toLocaleString()}
														inline
													/>
												)}
												{submission.lifecycle.adjudicationDeadline > 0n && (
													<MetaRow
														...
														value={new Date(
															Number(
																submission.lifecycle.adjudicationDeadline,
															) * 1000,
														).toLocaleString()}
														inline
													/>
									...
									{submission.grouping && (
										<div className="pt-4 border-t border-[var(--color-bg-light)]">
											...
											<div className="space-y-3">
												<MetaRow
													label="COHORT"
													value={submission.grouping.cohort}
													inline
												/>
												<MetaRow
													label="GROUP_ID"
													value={submission.grouping.groupId}
													inline
													valueClassName="text-[0.65rem] break-all text-[var(--color-text-dim)]"
												/>
												<MetaRow
													label="RANK_IN_GROUP"
													value={`${submission.grouping.groupRank} of ${submission.grouping.groupSize}`}
													inline
												/>
											</div>
										</div>
									...
									{submission.jury && (
										<div className="pt-4 border-t border-[var(--color-bg-light)]">
											...
											<div className="space-y-3">
												<MetaRow
													label="ACTION"
													value={submission.jury.action}
													inline
												/>
												{submission.jury.rationale && (
													<MetaRow
														label="RATIONALE"
														value={submission.jury.rationale}
														valueClassName="whitespace-normal mt-1 block text-[var(--color-text-dim)]"
													/>
												)}
											</div>
										</div>
						...
						<NeonPanel
							tone={hasActiveDispute ? "error" : "default"}
							contentClassName="p-4"
						>
							<h2
								className={`text-sm font-mono tracking-wider mb-3 ${hasActiveDispute ? "text-[var(--color-error)]" : "text-[var(--color-text)]"}`}
							>
							...
							{actualStatus === SUBMISSION_STATUS_FINALIZED ? (
								...
								<>
									<div className="space-y-3 font-mono text-sm mb-4">
										<div className="flex justify-between">
											...
											<span className="text-[var(--color-error)]">
												...
												>
													{submission.challenger.slice(0, 8)}...
													{submission.challenger.slice(-6)}
												</a>
											</span>
										</div>
										<div className="flex justify-between">
									...
									{canResolve ? (
											...
											<div className="flex gap-3">
												...
												>
													{actionLoading === "accept" ? (
														<>
															<span className="spinner w-3 h-3 mr-2 border-[var(--color-bg)] border-t-transparent" />{" "}
															ACCEPTING...
														</>
													) : (
														"ACCEPT (Uphold)"
										...
										<>
											<p className="text-[var(--color-text-dim)] font-mono text-xs mb-4">
												&gt; Dispute window has passed without resolution. Any
												connected address can finalize payout.
											</p>
											<Button
										...
										<p className="text-[var(--color-text-dim)] font-mono text-xs">
											...
											{!isConnected &&
												" (Connect wallet to finalize if timeout is reached)"}
										</p>
									)}
								</>
							) : canChallenge ? (
								<>
									<p className="text-[var(--color-text-dim)] font-mono text-xs mb-4">
										&gt; Challenge this verification result if you believe it's
										incorrect. Requires a minimum bond of{" "}
										{formatEther(challengeBond)} ETH.
									</p>
									<Button
								...
								<>
									<p className="text-[var(--color-text-dim)] font-mono text-xs mb-4">
										&gt; Dispute window has passed. Any connected address can
										finalize payout.
									</p>
									<Button
										onClick={handleFinalize}
				...
				<div className="mt-8 text-center">
					<Link to="/explorer">
						<Button
							variant="outline"
							className="border-[var(--color-primary)] text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-[var(--color-bg)] font-mono px-6 py-3"
						>
							[ BACK TO EXPLORER ]
						</Button>
					</Link>
